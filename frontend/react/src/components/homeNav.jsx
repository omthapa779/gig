...
const HomeNavbar = ({ handleSmoothScroll }) => {
  ...
  return (
    <>
      ...
      {/* Mobile Sidebar */}
      <div className={`fixed inset-0 z-[10050] bg-gray-900/50 transition-opacity duration-250 md:hidden ${sidebarOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} onClick={() => setSidebarOpen(false)} />
      
      <aside className={`home-sidebar fixed top-0 bottom-0 right-0 w-[min(82vw,340px)] site-surface shadow-2xl transition-transform duration-300 z-[10060] flex flex-col p-5 font-['Outfit'] md:hidden ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'}`} style={{ backgroundColor: 'var(--surface)', color: 'var(--text-primary)' }}>
